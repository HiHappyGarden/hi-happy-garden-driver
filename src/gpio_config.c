...
static struct hhgd_gpio_config* config = NULL;
...
{
    config = (struct hhgd_gpio_config*)_config;
...
bool hhg_lcd_pin_out_setup(u8 gpio_number, u8 gpio_direction, struct hhgd_error** error)
{
	...
	if( ret != 0 )	{
        ...
        snprintf(msg, sizeof(msg) - 1, "Failed to request GPIO %d \n", gpio_number);
	...
	if( ret != 0 )	{
        char msg[MSG_LEN_ERROR_HGD] = { [0 ... MSG_LEN_ERROR_HGD - 1] = 0};
        snprintf(msg, sizeof(msg) - 1, "Failed to export GPIO %d \n", gpio_number);
	...
	if( ret != 0 )	{
        char msg[MSG_LEN_ERROR_HGD] = { [0 ... MSG_LEN_ERROR_HGD - 1] = 0};
        snprintf(msg, sizeof(msg) - 1, "Failed to set GPIO direction %d \n", gpio_number );
